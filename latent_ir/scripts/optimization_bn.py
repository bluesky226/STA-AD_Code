...
def run_style_transfer_inr(cnn,network,optimizer, normalization_mean, normalization_std,
    ...
    prog_bar = tqdm(range(config_dict['mlp_iter']- config_dict['mlp_start_iter']))
...
def run_style_mlp(content, style, config_dict,epoch, model_weights):
    ...
    if (epoch>=1):
        ...
        network = configure_model_gnorm(network)
        params, names = collect_params(network)
        
        del optimizer
        optimizer = torch.optim.AdamW(params, lr=config_dict['mlp_out_lr'], betas=(config_dict['mlp_beta1'], config_dict['mlp_beta2']))
        previous_state = model_weights['model_state_dict']
        network.load_state_dict(previous_state)
        network = network.to(device)
        print('=======For this epoch, model is configured for Group norm and optimizer is also configured=======')
    ...
    if (epoch==0):
        model_weights['model_state_dict'] = network.state_dict()
    ...
    return outputs, result_np, img_1, model_weights
