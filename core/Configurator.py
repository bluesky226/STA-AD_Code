...
class DLConfigurator(object):
    ...
    def start_training_phi(self, global_model: dict = dict()):
        ...
        for idx, dst_name in enumerate(self.dl_config['downstream_tasks']):
            ...
            self.trainer.train_phi(model_state, opt_state, epoch)
    ...
    def start_evaluations(self, global_model):
        ...
        for idx, dst_name in enumerate(self.dl_config['downstream_tasks']):
            ...
            dst.start_task(global_model=global_model, params_dict=self.dl_config['test_time_hparams'])
            logging.info("[Configurator::eval]: ################ Finished downstream task nr. {}/{} ################"
                         .format(idx, nr_tasks))
