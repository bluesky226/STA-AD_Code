...
class PDownstreamEvaluator(DownstreamEvaluator):
    ...
    def start_task(self, global_model, params_dict=None):
        ...
        size_caller = config_dict['mlp_size_caller']
        
        print('=========The size caller is=========', size_caller)
        
        if 'smal' in size_caller:
            print('=====Calling small sizes for comutation=====')
            self.pathology_localization(global_model, 3, 71, True, params_dict)
        elif 'med' in size_caller:
            print('=====Calling mid sizes for comutation=====')
            self.pathology_localization(global_model, 71, 570, True,params_dict)
        elif 'lar' in size_caller:
    ...
    def pathology_localization(self, global_model, threshold_low, threshold_high, perc_flag=False, params_dict=None):
        ...
        SCR_FOLDER = '/lustre/groups/iml/projects/sameer.ambekar/ra/'
        optimization_config = dict()
        MODEL_DIR = os.path.join(SCR_FOLDER, wandb_run_name)
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                _, _, x_rec_dict_org = self.model.get_anomaly(x, x)

                x_rec_org = x_rec_dict_org['x_rec'] if 'x_rec' in x_rec_dict_org.keys() else torch.zeros_like(x)
                x_rec_org_theta = torch.clamp(x_rec_org, 0, 1)

                
                
                
                
                anomaly_map_theta, anomaly_score_theta, _ = self.model.get_anomaly_ir(x, x_rec_org_theta)
                ...
                x_rec_org_copy_np = x_rec_org_copy.cpu().detach().numpy()
                ...
                hist_img =  x_rec_org.detach().clone()
                ...
                to_visualize = [
                    {'title': 'x', 'tensor': x},
                    {'title': 'x_rec_theta', 'tensor': x_rec_org_theta},
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            wandb.log({f'Metrics/NEW_TP': tp_new})
            wandb.log({f'Metrics/NEW_FP': fp_new})
            
            tp_new = tp_new * 1000
            fp_new = fp_new * 1000
            tp_new = round(tp_new, 3)
            fp_new = round(fp_new, 3)
