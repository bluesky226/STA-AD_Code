    ...
    def pathology_localization(self, global_model, threshold_low, threshold_high, perc_flag=False, params_dict=None):
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                output_next = transforms.Grayscale(num_output_channels=1)(outputs[2])
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                to_visualize = [
                    ...
                    {'title': f'Saliency map theta {sal_value}', 'tensor': x_rec_org_saliency, 'cmap': 'plasma',
                        'vmax': sal_value},
                ...
                for i in range(len(x)):
                    ...
                    if 1==1:
                        ...
                        loss_mae  = torch.mean(torch.abs( x_rec_org[i][0].cpu().detach() - x_i.cpu().detach()))
                        test_metrics['MAE'].append(loss_mae.item())
                        loss_lpips = np.squeeze(lpips_alex(x_i.cpu(), x_rec_org[i][0].cpu()).detach().numpy())
                        test_metrics['LPIPS'].append(loss_lpips)
                        ssim_ = ssim(x_rec_org[i][0].cpu().detach().numpy(), x_i.cpu().detach().numpy(), data_range=1.)
                        test_metrics['SSIM'].append(ssim_)
