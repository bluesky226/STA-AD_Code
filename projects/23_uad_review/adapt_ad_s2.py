...
class PDownstreamEvaluator(DownstreamEvaluator):
    ...
    def pathology_localization(self, global_model, threshold_low, threshold_high, perc_flag=False, params_dict=None):
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            for idx, data in enumerate(dataset):
                ...
                outputs, outputs_np_img, o_img_1 = optimization_ir.run_style_mlp(context_img, style_img, config_dict, iter)
                alpha_value = int(config_dict['mlp_alpha_value'])
                
                
                
                output_next = outputs[alpha_value]
                
                output_next = transforms.Grayscale(num_output_channels=1)(output_next)
                ...
                to_visualize = [
                    ...
                    {'title': f'Saliency map theta {val_sal}', 'tensor': x_rec_org_saliency, 'cmap': 'plasma',
                        'vmax': val_sal},
                    
                    {'title': f'Saliency map ns {val_sal}', 'tensor': output_saliency, 'cmap': 'plasma',
                        'vmax': val_sal},
    ...
    def healthy_recons(self, global_model, threshold_low, threshold_high, perc_flag=False, params_dict=None):
        ...
        for dataset_key in self.test_data_dict.keys():
            ...
            MODEL_DIR = os.path.join('./saved_outputs_healthy_s2', wandb_run_name)
            ...
            for idx, data in enumerate(dataset):
                ...
                if load_image:
                    ...
                    folder_name = '/home/sameer/code/c-b/RA/saved_outputs_healthy_s2/daily-water-15/x_rec_images'
                    x_rec_org_copy_np = x_rec_org.cpu().detach().numpy()
                    ...
                    output_next = transform_load(img_path)
                    output_next = output_next.to(device)
                    output_next = output_next.unsqueeze(0)
                    ...
                    output_np = output_next.cpu().detach().numpy()
                    entropy_output = skimage.measure.shannon_entropy(output_np)
                    entropy_theta = skimage.measure.shannon_entropy(x_rec_org_copy_np)
                    entropy_input = skimage.measure.shannon_entropy(x_np)
                    
                    entropy_output_list.append(entropy_output)
                    entropy_theta_list.append(entropy_theta)
                    entropy_input_list.append(entropy_input)
                    ...
                    alpha_value = config_dict['mlp_alpha_value']
                    output_next = transforms.Grayscale(num_output_channels=1)(outputs[alpha_value])
                ...
                if save_img:
                    ...
                    f2 = os.path.join(fol2, fname)
                    x_rec2 = x_rec.cpu().detach().numpy()
                    x_rec2 = np.squeeze(x_rec2)
                    x_rec2 = x_rec2 * 255
                    x_rec2 = x_rec2.astype(np.uint8)
                    x_rec2 = Image.fromarray(x_rec2)
                    x_rec2.save(f2)
                    
                    
                    f3 = os.path.join(fol3, fname)
                    x_rec_s = x_rec.cpu().detach().numpy()
                    x_rec_s = np.squeeze(x_rec_s)
                    x_rec_s = x_rec_s * 255
                    x_rec_s = x_rec_s.astype(np.uint8)
                    x_rec_s = Image.fromarray(x_rec_s)
                    x_rec_s.save(f3)
                    print('Images saved')
            ...
            if load_image:
                print('List is ', entropy_output_list)
                print('Entropy mean of input is ', np.mean(entropy_input_list))
                print('Entropy mean of theta is ', np.mean(entropy_theta_list))
                print('Entropy mean of output is ', np.mean(entropy_output_list))
